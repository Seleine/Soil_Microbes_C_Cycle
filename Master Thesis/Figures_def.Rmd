...
fct_scatterplot <- function(y_of_choice, ylabel, x_of_choice, xlabel, reg_line, desired_ticks) {
  gg_scatterplot <-
    ...
    stat_cor(data = micro,
             ...
             colour = "black") +
    theme_bw() +
    theme(legend.position = "none", axis.title.y = element_text(size = 7), axis.title.x = element_text(size = 10),
          legend.text = element_text(size = 10), legend.title = element_text(size = 10))
...
PD2 <- subset(micro, moist != "D")
PD2_f <- subset(PD2, GlcN_f != 0)

gg_PD <- ggplot(data = PD2) +
  geom_point(aes(x = AS_abs, y = meso)) +
  labs(y = "Mesodensity Fraction in %", x = expression(paste("Amino Sugars (soil g"^"-1",")"))) +
  geom_smooth(method = lm, aes(x = AS_abs, y = meso), linetype = "dashed") +
  stat_cor(data = PD2, aes(y = meso, x = AS_abs, label=paste(..rr.label.., ..p.label.., sep = "~`,`~")), colour = "black", label.y = 6.5) +
  theme_bw() +
  theme(axis.title = element_text(size = 10), legend.text = element_text(size = 10),
        legend.title = element_text(size = 10))

gg_PD_f <- ggplot(data = PD2_f) +
  geom_point(aes(x = GlcN_f, y = meso)) +
  labs(y = "Mesodensity Fraction in %", x = expression(paste("Fungal Amino Sugars (soil g"^"-1",")"))) +
  geom_smooth(method = lm, aes(x = GlcN_f, y = meso)) +
  stat_cor(data = PD2_f, aes(y = meso, x = GlcN_f, label=paste(..rr.label.., ..p.label.., sep = "~`,`~")), colour = "black", label.y = 6.3) +
  theme_bw() +
  theme(axis.title = element_text(size = 10), legend.text = element_text(size = 10),
        legend.title = element_text(size = 10))

PD_plot <- ggarrange(gg_PD + rremove("ylab"), gg_PD_f + rremove("ylab"), nrow=2)
PD_plot <- annotate_figure(PD_plot, left = text_grob("Mesodensity Fraction in % (1.8-2.4 g/cc)", rot = 90, vjust = 1, size = 10))

gg_heatmap_PD <- ggarrange(heatmap_AS, gg_PD, heatmap_Community, gg_PD_f, ncol=2, nrow=2, labels = c("A", "C", "B", "D"))

# ggsave("gg_heatmap_PD_20231230.png", plot = gg_heatmap_PD, width=30, height=13, units="cm", dpi=1200)
```

#######################################################################################################################
Correlation Plot fungal AS and contribution C at temperature A4 and A5
```{r}
gg_AS_f_Cont_A4 <- ggplot(data = micro) +
  geom_point(aes(x = Contrib.A4_1, y = GlcN_f)) +
  labs(y = "fungal GlcN", x = expression(paste("Contribution A4 (%)"))) +
  geom_smooth(method = lm, aes(x = Contrib.A4_1, y = GlcN_f), linetype = "dashed") +
  stat_cor(data = micro, aes(y = GlcN_f, x = Contrib.A4_1, label=paste(..rr.label.., ..p.label.., sep = "~`,`~")), colour = "black", label.y = 200) +
  #xlim(0, NA) +
  theme_bw() +
  theme(axis.title = element_text(size = 10), legend.text = element_text(size = 10),
        legend.title = element_text(size = 10))

gg_AS_f_Cont_A5 <- ggplot(data = micro) +
  geom_point(aes(x = Contrib.A5_1, y = GlcN_f)) +
  labs(y = "fungal GlcN", x = expression(paste("Contribution A5 (%)"))) +
  #geom_smooth(method = lm, aes(x = GlcN_f, y = Contrib.A5_1), linetype = "dashed") +
  stat_cor(data = micro, aes(y = GlcN_f, x = Contrib.A5_1, label=paste(..rr.label.., ..p.label.., sep = "~`,`~")), colour = "black", label.y = 200) +
  #xlim(0, NA) +
  theme_bw() +
  theme(axis.title = element_text(size = 10), legend.text = element_text(size = 10),
        legend.title = element_text(size = 10))

AS_f_plot <- ggarrange(gg_AS_f_Cont_A4 + rremove("ylab"), gg_AS_f_Cont_A5 + rremove("ylab"), nrow=2, labels = c("A", "B"))
AS_f_plot <- annotate_figure(AS_f_plot, left = text_grob(expression(paste("Fungal Amino Sugars (soil g"^"-1",")")), rot = 90, vjust = 1, size = 10))

gg_AS_f_Cont_A4_A5 <- ggplot(data = micro) +
  geom_point(aes(x = (Contrib.A4_1 + Contrib.A5_1), y = GlcN_f)) +
  labs(y = expression(paste("Fungal Amino Sugars (soil g"^"-1",")")), x = "Contribution A4 and A5 (%)") +
  geom_smooth(method = lm, aes(x = (Contrib.A4_1 + Contrib.A5_1), y = GlcN_f), linetype = "dashed") +
  stat_cor(data = micro, aes(y = GlcN_f, x = (Contrib.A4_1 + Contrib.A5_1), label=paste(..rr.label.., ..p.label.., sep = "~`,`~")), colour = "black", label.y = 250) +
  #xlim(0, NA) +
  theme_bw() +
  theme(axis.title = element_text(size = 10), legend.text = element_text(size = 10),
        legend.title = element_text(size = 10))

ggsave("figures/gg_AS_f_Cont_A4_A5.png", plot = AS_f_plot, width=30, height=13, units="cm", dpi=1200)
ggsave("figures/gg_AS_f_Cont_A4_A5_sum.png", plot = gg_AS_f_Cont_A4_A5, width=30, height=13, units="cm", dpi=1200)
```
