...
fct_boxplot_moisture <- function(y_of_choice, y_label, ymax, x_label) {
  ...
  #                aes(x = Treatment, y = log(y_of_choice)),
  #                label = "p.format",
  #                label.x = 1, label.y = log(11000)) +
# annotate("text", x = 1, y = log(11500), label = "t-test", size = 5) +
  theme_bw()  +
  theme(legend.position = "none",
        axis.title = element_text(size = 12),
        axis.title.x=element_blank(),
...
#fct_tukey_two(micro$cum_C_h1_ng)
...
gg_AS3 <- ggarrange(
  fct_boxplot(micro$GlcN_C2, expression(paste("GlcN-C (μg C g"^"-1",") dry soil")), NA, "Community", y_log=FALSE, desired_ticks = NA),
  fct_boxplot(micro$ManN_C2, expression(paste("ManN-C (μg C g"^"-1",") dry soil")), NA, "Community", y_log=FALSE, desired_ticks = NA),
  fct_boxplot(micro$GalN_C2, expression(paste("GalN-C (μg C g"^"-1",") dry soil")), NA, "Community", y_log=FALSE, desired_ticks = NA),
  fct_boxplot(micro$MurN_C2, expression(paste("MurN-C (μg C g"^"-1",") dry soil")), NA, "Community", y_log=FALSE, desired_ticks = NA),
  nrow = 2, ncol = 2, common.legend = TRUE, legend = "bottom")

# , labels = c("A", "B", "C", "D")

# ggsave("figures/Fig_3_20250714_part1.png", plot = gg_AS3, width=16, height=13, units="cm", dpi=1200)
...
aov <- aov(data=micro, log(TOC1) ~ factor(Community) * factor(moist))
summary(aov)

aov <- aov(data=micro, log(TOC2) ~ factor(Community) * factor(moist))
summary(aov)
...
aov <- aov(data=micro, log(DOC_OD) ~ factor(Community) * factor(moist))
summary(aov)
```
...
aov <- aov(data=micro, log(remaining_glc_C_mg) ~ factor(Community) * factor(moist))
summary(aov)
```
